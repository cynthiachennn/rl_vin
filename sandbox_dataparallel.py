...
for epoch in range(epochs):
    ...
    data = torch.empty((2, 0, config['n_act'])) # stop using config['n_act'] !!!
    for world in worlds_train:
        ...
        for traj in range(n_traj):
            values = model(world.to(device), torch.Tensor(start_state).to(device), torch.Tensor(max_steps).to(device))
