...
class GridWorld:
    ...
    def set_vals(self):
        ...
        G = G[non_obstacles, :][:, non_obstacles] # adj matrix
        W = W[non_obstacles, :][:, non_obstacles] # transistion costs
...
def sample_start(M: GridWorld): # idk... returns the inital state # ... not the index of the state tho?
    G, W = M.get_graph_inv()
    N = G.shape[0]
    init_state = np.random.randint(0, N)
    return init_state
